...
def updatePlayerLocation(target,playerLocation):
    ...
    playerLocationx,playerLocationy=playerLocation
    nextMove=MOVE_UP
    if targety > playerLocationy:
        nextMove= MOVE_UP

    if targety < playerLocationy:
        nextMove= MOVE_DOWN

    if targetx > playerLocationx:
        nextMove= MOVE_RIGHT
        
    if targetx < playerLocationx:
        nextMove= MOVE_LEFT

    playerLocation=move(playerLocation,nextMove )
...
def checkEatCheese(playerLocation,opponentLocation,playerScore,opponentScore,piecesOfCheese):
    clone = piecesOfCheese.copy()
    for cheese in clone:
        if cheese == opponentLocation and cheese==playerLocation:
            opponentScore+=0.5
            playerScore+=0.5
            piecesOfCheese.remove(cheese)
        elif cheese==playerLocation:
            playerScore+=1
            piecesOfCheese.remove(cheese)
        elif cheese==opponentLocation:
            opponentScore+=1
            piecesOfCheese.remove(cheese)
...
def simulate_game_until_target(target,playerLocation,opponentLocation,playerScore,opponentScore,piecesOfCheese):
    ...
    while target in piecesOfCheese:
        ...
        opponentMove=turn_of_opponent(opponentLocation, piecesOfCheese)
        opponentLocation=move(opponentLocation, opponentMove)

...
def turn(mazeMap, mazeWidth, mazeHeight, playerLocation, opponentLocation, playerScore, opponentScore, piecesOfCheese, timeAllowed):
    if len(piecesOfCheese)<14:
        ...
        closest_poc = (-1,-1)
        closest_poc_opponent = (-1,-1)
        closest_poc_player = (-1,-1)
        best_distance = mazeWidth + mazeHeight
        best_distance_opponent = mazeWidth + mazeHeight
        
        #while target in piecesOfCheese:
            #listof= piecesOfCheese.copy()
        for poc in piecesOfCheese:
            #retrieve oppenent location
            if distance(poc, opponentLocation) < best_distance:
                best_distance_opponent = distance(poc, playerLocation)
                closest_poc_opponent = poc
                #retrieve player location
                #closest_poc=poc
            if distance(poc, playerLocation) < best_distance and distance(poc, playerLocation) <=best_distance_opponent:
